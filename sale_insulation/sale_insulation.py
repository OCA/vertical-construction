...
class sale_order_line(osv.osv):
    ...
    def write(self, cr, user, ids, vals, context=None):
        sale_order_line_obj = self.pool.get(
            'sale.order.line').browse(cr, user, ids, context=context)
        for line in sale_order_line_obj:
            product_obj = self.pool.get('product.product').browse(
                cr, user, line.product_id.id, context=context)
            if product_obj.sprayfoam:
                if 'rvalue' in vals and 'surface' in vals:
                    vals['product_uos_qty'] = vals['surface'] * \
                        vals['rvalue'] / line.product_rvalue
                else:
                    if 'rvalue' in vals:
                        vals['product_uos_qty'] = line.surface * \
                            vals['rvalue'] / line.product_rvalue
                    if 'surface' in vals:
                        vals['product_uos_qty'] = vals['surface'] * \
                            line.rvalue / line.product_rvalue
    ...
    def product_id_change(self,
        ...
        res = self.product_packaging_change(
            cr, uid, ids, pricelist, product, qty, uom, partner_id, packaging, context=context)
        ...
        compare_qty = float_compare(product_obj.virtual_available * uom2.factor, qty *
                                    product_obj.uom_id.factor, precision_rounding=product_obj.uom_id.rounding)
        if (product_obj.type == 'product') and int(compare_qty) == -1 \
                and (product_obj.procure_method == 'make_to_stock'):
            warn_msg = _('You plan to sell %.2f %s but you only have %.2f %s available !\nThe real stock is %.2f %s. (without reservations)') % \
                (qty, uom2 and uom2.name or product_obj.uom_id.name,
                 max(0,
                     product_obj.virtual_available), product_obj.uom_id.name,
    ...
    def uos_change(self, cr, uid, ids, product_uos, product_uos_qty=0, product_id=None):
        ...
        # FIXME must depend on uos/uom of the product and not only of the
        # coeff.
        try:
            value.update({
